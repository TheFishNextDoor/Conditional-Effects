    ...
    public static YamlConfiguration get(@NotNull String name, boolean copyMissingDefaults) {
        ...
        if (copyMissingDefaults) {
            ...
            try {
                ...
                ConditionalEffectsPlugin.logWarning("Failed to get default configuration for " + name + ".yml.");
                e.printStackTrace();
                return config;
            }
            ...
            if (changed) {
                ConditionalEffectsPlugin.logInfo("Added missing default values to " + name + ".yml.");
                save(name, config);
            }
        }
